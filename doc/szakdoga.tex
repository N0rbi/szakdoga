...
A nyelv szintaxisa megközelíti a pszeudokódét, például a hármas operátort a \ref{ternary} forráskódban látható módon definiáljuk, valamint sok vezérlési szerkezet helyettesítve lett.
\begin{figure}
	\label{ternary}
	\caption{A hármas if-then-else operátor pythonban}
	\centering
	\begin{BVerbatim}
	a = b if x else c
	\end{BVerbatim}
...
Az ebben a szekcióben leírtak megtalálhatóak a függelék \ref{train} részében.
...
Az utolsó paraméter pedig azt kérdezi milyen névem mentse a modell-t, hogy aztán késõbb beolvasható legyen, valamint hogy a tensorboard fel tudja dolgozni.
...
Ez viszont nem egy olyan megoldás amellyel hosszú távon érdemes egy mélytanuló algoritmust ellenõrizni.
Ezért döntöttem úgy hogy a TensorBoard-ot használom munkám során a fejlesztés könnyítéséhez.
...
\begin{table}[!h]\label{metrics}
	...
	\begin{center}
		\begin{tabular}{r|r}
			...
			val\_loss&0A validálás során számolt keresztentrópia átlaga\\
...
Ez a \textit{Gradient Descent} miatt van, próbálja megközelíteni a tanuló halmaz által reprezentált halmazt, így azt fogjuk észre venni, hogy a tanuló halmaz hibaértéke valamivel gyorsabban csökken, a pontossága nõ.
...
\section{Training}
\label{train}
\lstinputlisting[language=Python, breaklines=true]{../lyrics.py}
